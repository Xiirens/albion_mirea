...
async def main_1():
    """Создание словаря и ссылок для запросов к Git и AlbionDataProject"""
...
async def start(data_sorted, trans):
    """Запуск сортировки и выбор необходимых параметров"""
    Black = 'Black Market'
    time_update = 1440
    min_profit = 10
    money_profit = 20000
...
async def topografia(data, city_11, city_22, time_update, min_profit, money_profit, trans):
    """Сортировка полученных предметов"""
    for_data = []
    l = 0
    b = 0
    mess = ''
...
async def request_to_albion(items):
    """Дополнительный запрос Json Только по полученным предметам (для ускорения)"""
    now = datetime.now()
